...
export default function StartPage() {
  ...
  return (
    <main style={{ padding: "2rem", fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif", maxWidth: 720, margin: "0 auto" }}>
      <h1 style={{ fontSize: 20, fontWeight: 700, marginBottom: 6 }}>Upload our excel sheet to start auditing</h1>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8 }}>
        <Link href="/configs" style={{ background: '#b9d6df', padding: '6px 10px', borderRadius: 6 }}>manage configs</Link>
      </div>
      ...
      <section style={{ padding: "1rem", border: "1px dashed #bbb", borderRadius: 12, textAlign: 'center', marginBottom: 16 }}>
        ...
        {uploadId && (
          <div style={{ marginTop: 8, color: '#333' }}>Uploaded: <code>{uploadId}</code></div>
        )}
      </section>
      ...
      <section style={{ padding: "0.5rem 0" }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
          ...
          <Link href="/builder" style={{ background: '#b9d6df', padding: '4px 8px', borderRadius: 6 }}>+ Create new</Link>
        </div>
        <select value={presetName} onChange={(e) => setPresetName((e.target as HTMLSelectElement).value)} style={{ width: 320, padding: '6px 8px' }}>
      ...
      <div style={{ marginTop: 24 }}>
        <button disabled={!canProcess} style={{ background: canProcess ? '#b9d6df' : '#dbe7ec', padding: '10px 16px', borderRadius: 6, minWidth: 220, cursor: canProcess ? 'pointer' : 'not-allowed' }}>
          Let’s process →
        </button>
        {!canProcess && (
          <div style={{ color: '#666', marginTop: 6, fontSize: 12 }}>Upload a file and select a config to continue.</div>
        )}
      </div>
    </main>
  );
